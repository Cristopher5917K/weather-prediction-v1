...
void pedir_lecturas(Zona z[]) {
    ...
    for (int i = 0; i < ZONAS; ++i) {
        ...
        do {
            ...
            res = scanf("%f", &valor);
            while(getchar() != '\n'); // Limpiar buffer
            if (res != 1 || valor < 0 || valor > LIM_CO)
                printf("Valor invalido. Ingrese un numero positivo menor o igual a %.0f.\n", LIM_CO);
        } while (res != 1 || valor < 0 || valor > LIM_CO);
        z[i].act_CO = valor;
        ...
        do {
            ...
            res = scanf("%f", &valor);
            while(getchar() != '\n');
            if (res != 1 || valor < 0 || valor > LIM_NO2)
                printf("Valor invalido. Ingrese un numero positivo menor o igual a %.0f.\n", LIM_NO2);
        } while (res != 1 || valor < 0 || valor > LIM_NO2);
        z[i].act_NO2 = valor;
        ...
        do {
            ...
            res = scanf("%f", &valor);
            while(getchar() != '\n');
            if (res != 1 || valor < 0 || valor > LIM_PM25)
                printf("Valor invalido. Ingrese un numero positivo menor o igual a %.1f.\n", LIM_PM25);
        } while (res != 1 || valor < 0 || valor > LIM_PM25);
        z[i].act_PM25 = valor;
        ...
        do {
            ...
            res = scanf("%f", &valor);
            while(getchar() != '\n');
            if (res != 1 || valor < 0 || valor > LIM_SO2)
                printf("Valor invalido. Ingrese un numero positivo menor o igual a %.0f.\n", LIM_SO2);
        } while (res != 1 || valor < 0 || valor > LIM_SO2);
        z[i].act_SO2 = valor;
    }
}
...
int main() {
    ...
    float temp_CO[ZONAS][DIAS], temp_NO2[ZONAS][DIAS], temp_PM25[ZONAS][DIAS], temp_SO2[ZONAS][DIAS];
    float pred_CO[ZONAS][FUT], pred_NO2[ZONAS][FUT], pred_PM25[ZONAS][FUT], pred_SO2[ZONAS][FUT];

    int datos_cargados = 0, predicciones_cargadas = 0, lecturas_ingresadas = 0;
    int opcion;
    do {
        printf("\n=== MENU PRINCIPAL ===\n");
        ...
        switch(opcion) {
            case 1:
                ...
                datos_cargados = 1;
                predicciones_cargadas = 1;
                printf("Datos cargados correctamente.\n");
                break;
            case 2:
                if (!datos_cargados) {
                    printf("Primero debe cargar los datos historicos.\n");
                    break;
                }
                pedir_lecturas(z);
            ...
            case 3: {
                ...
                if (!ft || !fv || !fh) {
                    print_both("Error abriendo archivos meteorologicos para prediccion multiple.\n");
                } else {
            ...
            case 4:
                if (!lecturas_ingresadas) {
                    printf("Primero debe ingresar las lecturas actuales.\n");
                    break;
                }
                for (int i = 0; i < ZONAS; ++i)
                    procesar(&z[i], out);
